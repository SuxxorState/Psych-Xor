...
class PlayState extends MusicBeatState
{
	...
	var comboNumSize:Array<Int> = [91, 111];
	...
	override public function create()
	{
		...
		timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 244, 85, 400, "", 36);
		timeTxt.setFormat(Paths.font(hudFont), 36, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(chartingMode)
			timeTxt.text = Paths.formatToSongPath(SONG.song #if windows + '-' + storyDifficultyText #end);
		...
		missTxt = new FlxText(0, healthBarBG.y + 30, FlxG.width, "", 20);
		missTxt.setFormat(Paths.font(hudFont), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);	
		missTxt.scrollFactor.set();
		missTxt.x += FlxG.width / 7;
		missTxt.borderSize = 1.5;
		...
		chartingTxt.setFormat(Paths.font(hudFont), 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image(pixelShitPart1 + 'num' + pixelShitPart2), true, comboNumSize[0], comboNumSize[1]);
		...
		comboSpr.x = xThing + (PlayState.isPixelStage ? 75 : 50);
		/*
			trace(combo);
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if(daNote.gfNote && gf != null) char = gf;

		if(char != null && !daNote.noMissAnimation && char.hasMissAnimations && ((!daNote.isSustainNote && char.loopExists()) || !char.loopExists()))
		{
			var animToPlay:String = singAnimations[Std.int(Math.abs(daNote.noteData))] + 'miss' + daNote.animSuffix;
			char.playAnim(animToPlay, true, false, 2);	

			if (!daNote.isSustainEnd && !daNote.vibrato) char.pauseAnim();
		}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if (SONG.notes[curSection] != null && SONG.notes[curSection].altAnim) altAnim = '-alt';
		var animToPlay:String = singAnimations[Std.int(Math.abs(note.noteData))] + altAnim;

		if (char != null && !note.noAnimation && ((!note.isSustainNote && char.loopExists()) || !char.loopExists())) {
			switch(note.noteType) {
				case 'Hey!':
					char.playAnim(note.gfNote ? 'cheer' : 'hey', true, false, 0, true);
					char.heyTimer = 0.6;				
				default:
		...
		if (SONG.needsVoices /*&& SONG.seperateVoices*/) ovocals.volume = 1;

		var time:Float = 0.15;
		if(note.isSustainNote && !note.animation.curAnim.name.endsWith('end')) {
			time += 0.15;
		}
		StrumPlayAnim(true, Std.int(Math.abs(note.noteData)), time, note.isSustainEnd || note.vibrato);
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(note.hitCausesMiss) {
				...
				if(!note.noMissAnimation && char != null) {
					switch(note.noteType) {
						case 'Hurt Note': 
							char.playAnim('hurt', true, false, 0, true);
					}
				}
			...
			if(cpuControlled) {
				...
				StrumPlayAnim(false, Std.int(Math.abs(note.noteData)), time, note.isSustainEnd || note.vibrato);
			} else {
